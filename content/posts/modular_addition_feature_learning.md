...
### 4.2 The Mechanism: Majority Voting in Fourier Space
...
### 5.2 Neuron and Frequency Decoupling
...
### 5.4 Phase Alignment: The Zero-Attractor
...
The consequence of this cleanup is striking. Recall that during memorization (Figure 14), the white cells (truly unseen pairs where neither $(x,y)$ nor $(y,x)$ appeared in training) were completely wrong. After Stages II and III, those same pairs are now predicted correctly. This is because the cleaned-up network no longer relies on memorized associations. Instead, it implements the <span class="hl-orange">indicator function</span> from Proposition 1: the majority-voting mechanism in Fourier space computes $(x+y) \bmod p$ algorithmically for *any* input pair, including ones never seen during training. The network has transitioned from a lookup table to a generalizing algorithm. This is grokking.
